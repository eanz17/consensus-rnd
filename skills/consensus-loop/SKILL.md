...
| `scripts/codex_refactor_loop/controller_topology_authority.py` | canonical `<type>/YYYY-MM-DD_<purpose>` branch/worktree identity plus read-only `refactor/iter<I>-<cluster>` migration parsing | `ControllerTopologyAuthority.create_compliant_worktree()` is the sole branch/worktree writer and advances immutable provenance under fresh #191 CAS. `parse_legacy_implementation_head_evidence()` is the sole legacy grammar reader; legacy identity never gains write or backfill authority. |
...
### Temporary fork-only issue #2737 partial decomposition recovery
Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#issue-2737-partial-decomposition-recovery`. This disposable surface requires two independent keys: reviewed code owns a closed literal capability, while a separately reviewed immutable occurrence record owns incident/baseline/binding facts. Existing owner-local projections remain the only owners of mutable review, checks, merge, lease, actor, item admission, target state, and canonical #403 facts. Neither key authorizes anything alone. Recovery requires an empty conflict-free `IssueDecompositionApplyProjection`, performs at most one literal effect after two equal complete bindings and immediate revalidation, and immediately performs a fresh complete exact-prefix read after an ambiguous transport result; only the exact next public prefix adopts the effect and every other observation fails closed before another effect. It has no receipt, ticket, ledger, write-back, public CLI, daemon, wakeup action, host knob, generic executor, compatibility wrapper, compensation, or rollback. The occurrence record is intentionally absent from the capability PR. A separately reviewed cleanup must be the exact subtractive reverse recorded by the retained cleanup contract: delete every whole-file owner and occurrence record, reverse every bounded shared owner, add/move/rename/copy nothing, restore the frozen base bytes, and retain only the reviewed generic #403 projection/execution behavior and its tests.
...
5. `IMPLEMENT_DONE:ok` means the controller may inspect diff, commit, restore integration base, and queue or harvest the helper-private `publish_verification.py` receipt; only owner-private `publish_exact_head` may publish the verified exact SHA, finalize exactly one canonical managed PR plus receipt/provenance terminal, and then permit separate reviewer dispatch.
...
| IMPLEMENT_DONE:ok | controller commit + `publish_verification.py` job/receipt → owner-private `publish_exact_head` terminal → separate Consensus-rnd Phase review-gate reviewer × 3 |
...
3. **Publish implementation branch** through `publish_implementation_output`:
   the helper queues or harvests the `publish_verification.py` job/receipt, then gives the revalidated `VERIFIED` receipt and literal verified object id to owner-private `ControllerTopologyAuthority.publish_exact_head`; that transaction owns non-force exact-ref publication, remote equality proof, canonical PR create/update/adoption, and receipt finalization.
